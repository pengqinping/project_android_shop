...
public class Utils {
	...
	private static Bundle decodeUrl(String s) {
		...
		if (s != null) {
			...
			for (String parameter : array) {
				...
				try {
					params.putString(
							URLDecoder.decode(v[0], Constants.DEFAULT_CHARSET),
	 ...
	 * 
	 * @param url
	 *            需要打开的URL,必须为一个有效的URL
	 * @param method
	 *            使用的HTTP调用方法 ("GET", "POST", 等)
	 * @param params
	 *            查询参数 (例如 appKey=foo)
	 ...
	 */
	public static String openUrl(String url, String method, Bundle params,
			boolean multipart) throws MalformedURLException, IOException {
		...
		if (!Constants.GET_METHOD.equals(method)) {
			...
			if (multipart) {
				os.write(("--" + strBoundary + endLine)
						.getBytes(Constants.DEFAULT_CHARSET));
				os.write((encodePostBody(params, strBoundary))
						.getBytes(Constants.DEFAULT_CHARSET));
				os.write((endLine + "--" + strBoundary + endLine)
						.getBytes(Constants.DEFAULT_CHARSET));
			} else {
				os.write(buildQuery(params, Constants.DEFAULT_CHARSET)
						.getBytes(Constants.DEFAULT_CHARSET));
			}
		...
		if (header != null
				&& header.toLowerCase().contains(Constants.ACCEPT_ENCODING)) {
			isGzip = true;
		}
		String response = "";
		try {
	 ...
	 *             如果网络问题发生
	 */
	private static String read(InputStream in, String charset, boolean isGzip)
			throws IOException {
		StringBuilder sb = new StringBuilder();
		if (isGzip) {
		...
		BufferedReader r = new BufferedReader(
				new InputStreamReader(in, charset), 1000);
