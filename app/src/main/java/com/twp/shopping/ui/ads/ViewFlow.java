 ...
 */
package com.twp.shopping.ui.ads;
...
public class ViewFlow extends AdapterView<Adapter> {
	...
	private SwitchDirection mCurrentDirction = SwitchDirection.LEFT;
	 ...
	 *           VelocityTracker.obtain(); } mVelocityTracker.addMovement(ev);
	 * 
	 *           final int action = ev.getAction(); final float x = ev.getX();
	 * 
	 *           switch (action) { case MotionEvent.ACTION_DOWN: // If being
	 *           flinged and user touches, stop the fling. isFinished // will be
	 *           false if being flinged. if (!mScroller.isFinished()) {
	 *           mScroller.abortAnimation(); }
	 * 
	 *           // Remember where the motion event started mLastMotionX = x;
	 * 
	 ...
	 *           break;
	 * 
	 *           case MotionEvent.ACTION_MOVE: final int deltaX = (int)
	 *           (mLastMotionX - x);
	 * 
	 *           boolean xMoved = Math.abs(deltaX) > mTouchSlop;
	 * 
	 *           if (xMoved) { // Scroll if the user moved far enough along the
	 *           X axis mTouchState = TOUCH_STATE_SCROLLING;
	 * 
	 *           if (mViewInitializeListener != null) initializeView(deltaX); }
	 * 
	 *           if (mTouchState == TOUCH_STATE_SCROLLING) { // Scroll to follow
	 *           the motion event
	 * 
	 *           mLastMotionX = x;
	 * 
	 *           final int scrollX = getScrollX(); if (deltaX < 0) { if (scrollX
	 *           > 0) { scrollBy(Math.max(-scrollX, deltaX), 0); } } else if
	 *           (deltaX > 0) { final int availableToScroll = getChildAt(
	 *           getChildCount() - 1).getRight() - scrollX - getWidth(); if
	 *           (availableToScroll > 0) { scrollBy(Math.min(availableToScroll,
	 *           deltaX), 0); } } return true; } break;
	 * 
	 *           case MotionEvent.ACTION_UP: if (mTouchState ==
	 *           TOUCH_STATE_SCROLLING) { final VelocityTracker velocityTracker
	 *           = mVelocityTracker;
	 *           velocityTracker.computeCurrentVelocity(1000, mMaximumVelocity);
	 *           int velocityX = (int) velocityTracker.getXVelocity();
	 * 
	 *           if (velocityX > SNAP_VELOCITY && mCurrentScreen > 0) { // Fling
	 *           hard enough to move left snapToScreen(mCurrentScreen - 1); }
	 *           else if (velocityX < -SNAP_VELOCITY && mCurrentScreen <
	 *           getChildCount() - 1) { // Fling hard enough to move right
	 *           snapToScreen(mCurrentScreen + 1); } else { snapToDestination();
	 *           }
	 * 
	 ...
	 * 
	 *           break; case MotionEvent.ACTION_CANCEL: mTouchState =
		...
		case MotionEvent.ACTION_UP:
			...
			if (mTouchState == TOUCH_STATE_SCROLLING) {
				...
				if (velocityX > SNAP_VELOCITY && mCurrentScreen > 0) {
					...
					mCurrentDirction = SwitchDirection.LEFT;
					Logger.i("MOVE TO LEFT ! chang is direction ");
				} else if (velocityX < -SNAP_VELOCITY
						&& mCurrentScreen < getChildCount() - 1) {
					// Fling hard enough to move right
					snapToScreen(mCurrentScreen + 1);
					mCurrentDirction = SwitchDirection.RIGHT;
					Logger.i("MOVE TO RIGHT ! chang is direction ");
				} else {

					//点击事件在这里处理
					final float down2UpDelayX = ev.getX() - mTouchDownX;
					final float down2UpDelayY = ev.getY() - mTouchDownY;
					if (Math.abs(down2UpDelayX) < 10) {
						getOnItemClickListener().onItemClick(this,
								getChildAt(mCurrentScreen), mCurrentScreen, 0l);
					}
					snapToDestination();
				}
	...
	private void snapToDestination() {
		...
		final int whichScreen = (getScrollX() + (screenWidth / 2))
				/ screenWidth;
		if (whichScreen - mCurrentScreen > 0) {
			Logger.i("MOVE change dircetion to right!");
			mCurrentDirction = SwitchDirection.RIGHT;
		} else if (whichScreen - mCurrentScreen < 0) {
			Logger.i("MOVE change dircetion to left!");
			mCurrentDirction = SwitchDirection.LEFT;
		} else {
			Logger.i("MOVE not change.");
		}
